...
# Variable Prize
# Imputamos la media en train más adelante
...
raceruns <- raceruns %>% select(-win_dividend2, -win_combination2, -time7, 
                                ...
                                -behind_sec5,-behind_sec4, -place_dividend3, -place_combination3)
y_final <- raceruns%>% filter(train_val_test =="test")
...
# Chequeamos que no contamos con missing values
sum(is.na(raceruns))
...
for(i in race_id){
  ...
  #raceruns$relative_position[sel] <- raceruns$result[sel]/raceruns$n_compet[sel]          #no la tomamos en cuenta (posible data leakage)
  #raceruns$horsepower[sel] <- raceruns$declared_weight[sel]/raceruns$finish_time[sel]     #no la tomamos en cuenta (posible data leakage)
}
...
# Agrupamos la variable horse_ratings
...
sum(is.na(raceruns$ratings_groups)) #Check de que no nos falte agrupar nada
table(raceruns$horse_ratings)
...
# No tomamos en cuenta lo siguiente (posible data leakage)
...
str(raceruns)
cols_to_delete <- c("result", "race_id", "lengths_behind" ,"horse_no", "position_sec3",  
                    "position_sec1", "position_sec2","behind_sec1","behind_sec2",
                    "behind_sec3","time1.x","time2.x", "time3.x","time1.y","time2.y",
                    "time3.y","finish_time","sec_time1","sec_time2",
                    "sec_time3","place_combination1","place_combination2",
                    "place_dividend1","place_dividend2","win_combination1",
                    "win_dividend1", "date", "day","month","year")

raceruns <- raceruns %>% select(-cols_to_delete)


#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Separamos el dataset en training, validation y testing 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
# Ahora si, imputamos la media a prize como anteriormente mencionamos
...
for(i in horse_id){
  ...
  train_set$is_horse_xp[sel]  <- ifelse(length(sel)>30,1,0) # Tomamos valores medios como umbral mediante inspección de la tabla
}
...
for(i in jockey_id){
  ...
  train_set$is_jockey_xp[sel]  <- ifelse(length(sel)>2000,1,0) # Tomamos valores medios como umbral mediante inspección de la tabla
}
...
for(i in trainer_id){
  ...
  train_set$is_trainer_xp[sel]  <- ifelse(length(sel)>2000,1,0) # Tomamos valores medios como umbral mediante inspección de la tabla
}
...
# Creamos estos features para validacion (usando data de train). De esta manera,
# aprovechamos la información histórica dinámica sin cometer data leakage. Ver 
# justificación en el infome.

horse_id_v <- unique(val_set$horse_id) # id único de cada caballo
jockey_id_v <- unique(val_set$jockey_id) # id único de cada jockey
trainer_id_v <- unique(val_set$trainer_id) # id único de cada trainer
...
# Creamos estos features para test (usando data de train). De esta manera,
# aprovechamos la información histórica dinámica sin cometer data leakage. Ver 
# justificación en el infome.

horse_id_t <- unique(test_set$horse_id) # id único de cada caballo
jockey_id_t <- unique(test_set$jockey_id) # id único de cada jockey
trainer_id_t <- unique(test_set$trainer_id) # id único de cada trainer
...
# Vemos que las proporciones guardan lógica debido a que es información estática
...
#de los que no imputamos la media (no sería problemático porque son pocas observaciones)

train_set = na.omit(train_set)
test_set = na.omit(test_set)
val_set = na.omit(val_set)
y_final =na.omit(y_final)
...
umbral = c (0.07,0.08,0.1,0.11,0.12,0.13,0.14,0.15,0.16)
...
# 4) Modelo Baseline (Árbol de decisión) ####
...
umbral = c (0.001, 0.01, 0.02,0.025,0.035,0.04,0.05,0.07)
...
#es top 3? 1 si es top3 o 0 si no
y_final$is_top3 <- ifelse(
  y_final$result == 1 | y_final$result == 2 | y_final$result == 3, 1, 0)
table(raceruns$is_top3); prop.table(table(raceruns$is_top3)) 

#~~8.1) Primera estrategia usando el mejor modelo (inversión aversa al riesgo)----
# hacemos pocas apuestas
#Entrenamos modelo
y_pred <- predict(xgb2, test_set %>% select(-won), type = "prob")[, 2]
...
#~~8.2) Segunda estrategia bajando el umbral (inversión amante al riesgo)----
...
#Agregamos una col mas donde vemos si es top 3 o no
#es top 3? 1 si es top3 o 0 si no
y_final_val$is_top3 <- ifelse(
  y_final_val$result == 1 | y_final_val$result == 2 | y_final_val$result == 3, 1, 0)


#Creamos vector vacio y umbral
beneficio<- c()
umbral= c(0.2,0.4,0.6,0.8)

for (i in 1:length(umbral)){
  y_pred <- predict(xgb, val_set %>% select(-won), type = "prob")[, 2]
  y_pred <- ifelse(y_pred>umbral[i],1,0)
  y_val <- val_set$won
  y_final_val$apuesta <- y_pred
  
  y_final_val$ganancia_ganador <- ifelse(y_final_val$won==1 & y_final_val$apuesta==1, y_final_val$win_odds,0)
  y_final_val$ganancia_podio <- ifelse(y_final_val$is_top3==1 & y_final_val$apuesta==1, y_final_val$place_odds,0)
  y_final_val$ganancia_total <- y_final_val$ganancia_ganador+y_final_val$ganancia_podio
  
  #Obtenemos retorno por dolar
  retorno_por_dolar <- (sum(y_final_val$ganancia_total)/sum(y_final_val$apuesta)-1)*100
  beneficio <- append(beneficio,retorno_por_dolar)
...
#~~8.4) Apuesta final sobre test con umbral optimizado (inversión moderada frente al riesgo) ----
...
#~~8.5) Apostando distintos montos dependiendo de la probabilidad (inversión diversificada) ----

y_pred <- predict(xgb2, test_set %>% select(-won), type = "prob")[, 2]

y_final$apuestadoble <- ifelse(y_pred>0.55,1,0)
y_final$apuestasimple <- ifelse(y_pred>0.35,1,0)
