...
# GRÁFICO_6 (chequear porque no me sale)#### 
library(ggridges)
library(ggplot2)
library(viridis)
library(hrbrthemes)

caballos_ganadoresIII <- raceruns %>%
  filter(won == 1) %>%
  group_by(horse_id, horse_age) %>%
  summarise(Cant_carreras_ganadas = sum(won)) 

# Plot
ggplot(caballos_ganadoresII, aes(x = Cant_carreras_ganadas, y = horse_age, fill = stat(x))) +
...
# GRÁFICO_11 (misma data que el 10) ####
table(raceruns$horse_age); prop.table(table(raceruns$horse_age))
...
x11(); ggplot(raceruns, aes(x = result, y = horse_type, fill = horse_type)) +
  geom_density_ridges() +
  theme_ridges() + 
