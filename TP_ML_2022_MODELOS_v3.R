...
raceruns$horse_age <- scale(raceruns$horse_age)
raceruns$horse_rating <- scale(raceruns$horse_rating)
raceruns$win_odds <- scale(raceruns$win_odds)
raceruns$place_odds <- scale(raceruns$place_odds)
raceruns$prize <- scale(raceruns$prize)
raceruns$weight_ratio <- scale(raceruns$weight_ratio)
...
train_set <- train_set%>%select(win_odds,place_odds,won)

train_set_one_hot_encoding <- one_hot(as.data.table(train_set))
gc()
...
svmfit <- svm(y_train~., 
              ...
              cost = 0.01)   # C en las slides.        
svmfit
x11()
plot(svmfit, train_set) # Pifiamos un poco en el centro (x1 = 0, x2 = 0).
